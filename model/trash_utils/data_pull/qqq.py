...
def get_stock_for_month(date: datetime.date, ticker):
    ...
    print(date_str)
    print(next_month)
    resp = requests.get(f"https://api.polygon.io/v2/aggs/ticker/{ticker}/range/1/minute/{date_str}/{next_month}?adjusted=true&sort=asc&limit=50000&apiKey=uP_Wh9PpaphGqy1ZMOHgR10dC3jcC2cY")
    return resp.json()
...
def get_stock_data_date_range(ticker: str, date_start: datetime.date, date_end: datetime.date):
    d = date_start
    while d <= date_end:
